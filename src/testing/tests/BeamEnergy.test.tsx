...
vi.mock('@/hooks/useOphydDeviceSocket', () => ({
  default: vi.fn(() => ({
    devices: {
      mono_energy: {
        name: 'mono_energy', value: 3000, units: 'eV',
        pv: 'mono_energy', timestamp: 0, connected: true,
        read_access: true, write_access: true, locked: false, expanded: false,
      },
    },
...
const makeDevice = (overrides: Partial<Device> = {}): Device => ({
  pv: 'TEST:PV', name: 'TEST:PV', value: 15.5,
  timestamp: 0, connected: true, read_access: true, write_access: true,
  locked: false, expanded: false,
  ...overrides,
});

const defaultHeaderProps = {
  title: 'Beam Energy', pv: 'TEST:PV',
  showController: false, showPlot: false, showAbout: false, isLocked: false,
  handleToggleLock: noop, handleToggleController: noop,
  handleTogglePlot: noop, handleToggleAbout: noop,
};

beforeEach(() => { ScatterMock.mockClear(); vi.useFakeTimers(); });
...
describe('BeamEnergyAbout', () => {
  ...
  it('applies className to the pre element', () => {
    const { container } = render(<BeamEnergyAbout device={makeDevice()} className="overflow-auto" />);
...
describe('BeamEnergyHeader', () => {
  ...
  it('calls handleToggleController when joystick icon is clicked', () => {
    ...
    render(<BeamEnergyHeader {...defaultHeaderProps} handleToggleController={handleToggleController} />);
    const svgs = document.querySelectorAll('svg');
...
describe('BeamEnergyController', () => {
  ...
  it('calls onRelativeMove on Enter key in Jog Energy mode', () => {
    ...
    fireEvent.change(screen.getByTestId('move-mode-select'), { target: { value: 'Jog Energy' } });
    const input = screen.getByRole('spinbutton');
...
describe('BeamEnergyPV (demo mode)', () => {
  ...
  it('moves toward target energy in demo mode over time', () => {
    ...
    act(() => { vi.advanceTimersByTime(600); }); // 600ms → partway through 1000ms move
    const evText = screen.getByText(/eV/);
    const value = parseFloat(evText.textContent!);
