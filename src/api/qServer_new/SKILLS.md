...
hooks/
  types.ts                 QServerQueryHookOptions / QServerMutationHookOptions
...
2. Write the hook in the matching `hooks/<domain>Hooks.ts`, with its per-hook options interface.
3. For a mutation, add its bundle list to `QSERVER_MUTATION_INVALIDATIONS`.
4. Export the hook and its options type from `hooks/index.ts`.
