...
vi.mock('@/hooks/useOphydSocket', () => ({
  default: vi.fn(() => ({ devices: {}, handleSetValueRequest: vi.fn(), toggleDeviceLock: vi.fn() })),
}));
...
const makeDevice = (axis: string, value = 0): Device => ({
  pv: `demo:${axis}`, name: `demo:${axis}`, value,
  timestamp: 0, connected: true, read_access: true, write_access: true,
  locked: false, expanded: false, units: 'mm',
});
...
const defaultHeaderProps = {
  showController: true, showPlot: false, showAbout: false, isLocked: false,
  onClickLock: noop, onClickController: noop, onClickPlot: noop, onClickAbout: noop,
};
...
describe('HexapodController', () => {
  ...
  it('calls onStartClick with form values when START is clicked', () => {
    ...
    render(
      <HexapodController hexapodRBVs={makeRBVs()} onStartClick={onStartClick} onStopClick={noop} />,
    );
  ...
  it('calls onStopClick when STOP is clicked', () => {
    ...
    render(
      <HexapodController hexapodRBVs={makeRBVs()} onStartClick={noop} onStopClick={onStopClick} />,
    );
  ...
  it('shows jog hint text in Jog mode instead of START/STOP buttons', () => {
    render(<HexapodController hexapodRBVs={makeRBVs()} onStartClick={noop} onStopClick={noop} />);
  ...
  it('does not call onStartClick on Enter in Jog mode when isLocked', () => {
    const onStartClick = vi.fn();
    render(
      <HexapodController hexapodRBVs={makeRBVs()} onStartClick={onStartClick} onStopClick={noop} isLocked />,
    );
    fireEvent.change(screen.getByTestId('move-mode-select'), { target: { value: 'Jog' } });
    const inputs = screen.getAllByRole('textbox');
...
describe('Hexapod (demo mode)', () => {
  ...
  it('reaches the target value after the full move duration', () => {
    ...
    act(() => { vi.advanceTimersByTime(3100); }); // 3 mm at 1 mm/s = 3000 ms
  ...
  it('stops a move in progress when STOP is clicked', () => {
    ...
    act(() => { vi.advanceTimersByTime(2000); }); // midway through 10 s move
    const midValues = screen.getAllByText(/mm/).map((el) => parseFloat(el.textContent!));
    const midTx = midValues[0];

    fireEvent.click(screen.getByText('STOP'));
    act(() => { vi.advanceTimersByTime(3000); }); // advance more — should not change
  ...
  it('clamps moves to ±10 mm', () => {
    ...
    act(() => { vi.advanceTimersByTime(15000); }); // well past 10 s cap
