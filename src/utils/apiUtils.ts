...
export function useOphydApiUrls() {
  ...
  const httpBaseUrl = config?.ophydApiUrl || `http://${currentAddress}:${DEFAULT_OPHYD_API_PORT}/api/v1`;
  const wsBaseUrl = config?.ophydApiUrl ? httpToWsUrl(config.ophydApiUrl) : `ws://${currentAddress}:${DEFAULT_OPHYD_API_PORT}/api/v1`;
...
export function useQueueServerApiUrls() {
  const config = useOptionalFinchConfig();
  const httpBaseUrl = config?.qServerApiUrl || `http://${currentAddress}:${DEFAULT_QSERVER_API_PORT}/api`;
...
export function useTiledApiUrls() {
  const config = useOptionalFinchConfig();
  const httpBaseUrl = config?.tiledApiUrl || `http://${currentAddress}:${DEFAULT_TILED_API_PORT}/api/v1`;
