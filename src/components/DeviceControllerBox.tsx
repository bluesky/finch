...
export default function DeviceControllerBox({ device, deviceRBV, handleSetValueRequest, handleLockClick, svgIcon, className, title, ...props}: DeviceControllerBoxProps) {
    ...
    return (
        <article className={cn(`w-96 h-fit border border-slate-300 shadow-lg rounded-lg flex flex-col relative bg-slate-200 text-slate-900`, className)} {...props}>
            {/*Row -Icons */}
            ...
            {/* Row - Absolute move */}
            <div className={`${device.locked ? 'opacity-60' : ''} flex justify-around items-center py-2`}>
            ...
            {/* Row - Relative Move */}
            <div className={`${device.locked ? 'opacity-60' : ''} py-4 flex justify-center space-x-4`}>
                <div 
