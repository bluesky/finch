...
const makeDevices = (entries: [string, Partial<Device>][]): Devices =>
  Object.fromEntries(entries.map(([name, overrides]) => [name, makeDevice({ name, ...overrides })]));
...
describe('TableDeviceController', () => {
  ...
  it('renders without crashing with no devices', () => {
    const { container } = render(
      <TableDeviceController devices={{}} handleSetValueRequest={noop} toggleDeviceLock={noop} toggleExpand={noop} />
    );
    expect(container.firstChild).toBeInTheDocument();
  });

  it('renders a row for each device', () => {
    const devices = makeDevices([['motor1', {}], ['motor2', {}]]);
    render(<TableDeviceController devices={devices} handleSetValueRequest={noop} toggleDeviceLock={noop} toggleExpand={noop} />);
    expect(screen.getByText('motor1')).toBeInTheDocument();
    expect(screen.getByText('motor2')).toBeInTheDocument();
  });

  it('renders column headers', () => {
    render(<TableDeviceController devices={{}} handleSetValueRequest={noop} toggleDeviceLock={noop} toggleExpand={noop} />);
  ...
  it('calls toggleExpand with device name when name cell is clicked', () => {
    ...
    render(<TableDeviceController devices={devices} handleSetValueRequest={noop} toggleDeviceLock={noop} toggleExpand={toggleExpand} />);
  ...
  it('calls handleSetValueRequest when absolute move is submitted', () => {
    ...
    const devices = makeDevices([['motor1', {}]]);
    render(<TableDeviceController devices={devices} handleSetValueRequest={handleSetValueRequest} toggleDeviceLock={noop} toggleExpand={noop} />);
  ...
  it('applies className to the root container', () => {
    const { container } = render(
      <TableDeviceController devices={{}} handleSetValueRequest={noop} toggleDeviceLock={noop} toggleExpand={noop} className="my-table" />
    );
