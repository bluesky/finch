...
vi.mock('@/assets/icons', () => ({
  phosphorIcons: { eye: null, eyeSlash: null, plugs: null, plugsConnected: null, camera: null, cameraSlash: null },
...
describe('InputField', () => {
  it('renders without crashing', () => {
    const { container } = render(
      <InputField onSubmit={vi.fn()} pv="test:pv" input={{ suffix: 'AcquireTime', label: 'Acquire Time', type: 'float' }} cameraSettingsPVs={{}} />,
    );
    expect(container.firstChild).toBeInTheDocument();
  });
  ...
  it('shows "Not Connected" when PV is absent from cameraSettingsPVs', () => {
    render(
      <InputField onSubmit={vi.fn()} pv="missing:pv" input={{ suffix: 'AcquireTime', label: 'Acquire Time', type: 'float' }} cameraSettingsPVs={{}} />,
    );
...
describe('InputGroup', () => {
  ...
  it('renders without crashing', () => {
    const { container } = render(
      <InputGroup settingsGroup={group} prefix="13SIM1" cameraSettingsPVs={{}} onSubmit={vi.fn()} />,
    );
    expect(container.firstChild).toBeInTheDocument();
  });

  it('shows the group title', () => {
    render(<InputGroup settingsGroup={group} prefix="13SIM1" cameraSettingsPVs={{}} onSubmit={vi.fn()} />);
...
describe('CameraControlPanel', () => {
  ...
  it('renders without crashing', () => {
    const { container } = render(
      <CameraControlPanel cameraControlPV={connectedPV} startAcquire={vi.fn()} stopAcquire={vi.fn()} />,
    );
    expect(container.firstChild).toBeInTheDocument();
  });

  it('shows current acquisition status', () => {
    render(<CameraControlPanel cameraControlPV={connectedPV} startAcquire={vi.fn()} stopAcquire={vi.fn()} />);
    expect(screen.getByText(/Acquisition Status: Done/)).toBeInTheDocument();
  });

  it('disables buttons when PV is not connected', () => {
    render(<CameraControlPanel cameraControlPV={disconnectedPV} startAcquire={vi.fn()} stopAcquire={vi.fn()} />);
  ...
  it('calls startAcquire when Acquire button is clicked', () => {
    ...
    render(<CameraControlPanel cameraControlPV={connectedPV} startAcquire={startAcquire} stopAcquire={vi.fn()} />);
...
describe('CameraCanvasFeatures', () => {
  it('renders without crashing', () => {
    const { container } = render(
      <CameraCanvasFeatures socketStatus="closed" isImageLogScale={false} onToggleConnection={vi.fn()} onToggleLogScale={vi.fn()} />,
    );
  ...
  it('shows "Log Scale" title when not in log scale', () => {
    render(
      <CameraCanvasFeatures socketStatus="open" isImageLogScale={false} onToggleConnection={vi.fn()} onToggleLogScale={vi.fn()} />,
    );
  ...
  it('shows "Linear Scale" title when in log scale', () => {
    render(
      <CameraCanvasFeatures socketStatus="open" isImageLogScale={true} onToggleConnection={vi.fn()} onToggleLogScale={vi.fn()} />,
    );
  ...
  it('calls onToggleLogScale when the scale icon is clicked', () => {
    ...
    render(
      <CameraCanvasFeatures socketStatus="open" isImageLogScale={false} onToggleConnection={vi.fn()} onToggleLogScale={onToggleLogScale} />,
    );
  ...
  it('calls onToggleConnection when the connection icon is clicked', () => {
    ...
    render(
      <CameraCanvasFeatures socketStatus="open" isImageLogScale={false} onToggleConnection={onToggleConnection} onToggleLogScale={vi.fn()} />,
    );
