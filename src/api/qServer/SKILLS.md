...
hooks/
  ...
  internal/                the one query engine, the one mutation engine
                           (request merging + the transport contract live in @/api/shared)
...
Call shape: positional — `useQueueXQuery(arg?, queryOptions?, requestOptions?)` and
`useQueueXMutation(mutationOptions?, requestOptions?)`. **`requestOptions` is always last**, per the
cross-backend convention stated in `@/api/shared/queryOptions`; the Tiled hooks are identical. The
argument slot exists only when the endpoint takes one; a required argument is typed required (accepting
`undefined` where the hook guards itself with `defaultEnabled`, and calling `requireArg` in `fetch` so
a forced `enabled: true` fails loudly rather than sending `undefined`). A JSON body parameter is always
named `body`. Mutation bodies go through `mutate(variables)`, never the hook.

The three socket hooks are the exception and take one options object — they are not TanStack hooks, so
there is no `queryOptions` slot to order transport against. See the README.
