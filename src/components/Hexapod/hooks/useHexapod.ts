...
type AxisMoveParams = { startPos: number; targetPos: number; startTime: number; durationMs: number; };
...
export default function useHexapod(props: UseHexapodProps = {}) {
    ...
    const setSimPos = (pos: SimPos) => { simPosRef.current = pos; setSimPosState(pos); };
    ...
    const handleStartClick = useCallback((movePositionForm: HexapodMovePositionForm, isRelative: boolean) => {
        if (demo) {
            ...
            AXES.forEach((axis) => {
                ...
                if (durationMs > 0) {
                    newMoves[axis] = { startPos, targetPos: target, startTime: now, durationMs };
                }
            });
            ...
            if (Object.keys(newMoves).length > 0) {
                ...
                moveIntervalRef.current = setInterval(() => {
                    ...
                    AXES.forEach((axis) => {
                        ...
                        if (!move) return;
                        const progress = Math.min((now - move.startTime) / move.durationMs, 1.0);
                        newPos[axis] = move.startPos + progress * (move.targetPos - move.startPos);
                        if (progress < 1.0) {
                            anyActive = true;
                        } else {
