...
export function useCameraContainer({ prefix, settings, enableControlPanel, cameraControlWsUrl }: UseCameraContainerProps) {
    const sanitizeInputPrefix = (p: string) => {
        const trimmed = p.trim();
        return trimmed.slice(-1) === ':' ? trimmed.substring(0, trimmed.length - 1) : trimmed;
    };

    const createControlPVString = useCallback((p = '') => {
    ...
    const createDeviceNameArray = useCallback((s: DetectorSetting[], p: string) => {
    ...
    const deviceNames = useMemo(() => createDeviceNameArray(settings, prefix), [createDeviceNameArray, settings, prefix]);
