...
export default function SignalMonitorPlotOphyd({ deviceName, ...props }: SignalMonitorPlotOphydProps) {
    const deviceNameList = useMemo(() => [deviceName], [deviceName]);
    const { devices } = useOphydDeviceSocket(deviceNameList);
    return <SignalMonitorPlotDevice device={devices[deviceName]} deviceLabel={deviceName} {...props} />;
}
