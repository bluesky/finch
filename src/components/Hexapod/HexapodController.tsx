                    ...
                    <ul>
                        {Object.keys(hexapodRBVs).map((axis) => (
                            <li key={axis} className="flex h-8">
                                ...
                                <p className="w-3/4">{hexapodRBVs[axis as keyof HexapodRBVs].connected ? formatCurrentValue(hexapodRBVs[axis as keyof HexapodRBVs]) : 'N/A'}</p>
                            </li>
                        ))}
                    </ul>
                </div>
                    ...
                    <ul>
                        {Object.keys(movePositionForm).map((axis) => (
                            <li key={axis} className="flex h-8 items-center">
                                ...
                                <input
                                    type="text"
                                    className="w-20 border border-slate-300 rounded-md pl-1"
                                ...
                                />
                                <p>{hexapodRBVs[axis as keyof HexapodRBVs].units}</p>
                            </li>
                        ))}
                    </ul>
                    <span className="flex">
