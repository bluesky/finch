...
export default function BeamEnergyOphyd({deviceName="mono_energy", title="Beam Energy", thetaOffsetDeg, wsUrl, className}:BeamEnergyOphydProps) {
    ...
    return (
        <section className={cn("flex flex-col w-64 h-72 bg-slate-200 rounded-lg shadow-lg p-4 min-w-[26rem] relative", className)}>
            ...
            {(showController && !showAbout && !showPlot) && <BeamEnergyController onAbsoluteMove={handleAbsoluteMove} onRelativeMove={handleRelativeMove} onStop={handleStop} isLocked={isLocked} />}
