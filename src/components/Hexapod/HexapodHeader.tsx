    ...
    return (
        <nav className="flex items-center justify-between">
            <div className="flex flex-shrink-0 w-fit gap-2">
                ...
                <Joystick size={24} className={`${showController && !showAbout ? "text-slate-900" : "text-slate-500"} hover:cursor-pointer hover:text-slate-700`}  onClick={onClickController}/>
            </div>
            <h2 className="text-sky-900 font-semibold text-md text-center w-full mb-2">{prefix}</h2> 
            <div className="flex flex-shrink-0 w-fit gap-2">
                <ChartLine size={24} className={`${showPlot && !showAbout ? "text-slate-900" : "text-slate-500"} hover:cursor-pointer hover:text-slate-700`} onClick={onClickPlot}/>
